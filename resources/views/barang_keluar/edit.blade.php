                    ...
                    <form method="POST" action="{{ route('barang-keluar.update', $barangKeluar->id) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('barang_id') is-invalid @enderror" id="barang_id" name="barang_id" required>
                                ...
                                @foreach($barang as $item)
                                    <option value="{{ $item->getKey() }}" {{ old('barang_id', $barangKeluar->barang_id) == $item->getKey() ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('tanggal') is-invalid @enderror" 
                                   id="tanggal" name="tanggal" value="{{ old('tanggal', optional($barangKeluar->tanggal_keluar)->format('Y-m-d')) }}" required>
                            @error('tanggal')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
